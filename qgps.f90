...
! -----  time stepping -------------
    ...
    subroutine stepping
      ...
      real(dp) :: dt2,dt3,dt6
      real(dp) :: tmp
      real(dp), dimension(n,n,n) :: epo, emo
...
!$omp parallel do private(ix,iy,iz) shared(epo,emo,rks)
      do iz=1,n
         do iy=1,n
            do ix=1,n
               epo(ix,iy,iz) = exp(dfac*rks(ix,iy,iz))
               emo(ix,iy,iz) = 1.d0/epo(ix,iy,iz)
            enddo
         enddo   
      enddo   
!$omp end parallel do

!$omp parallel do private(ix,iy,iz) shared(q,emo,psi,qi,qf)
      do iz=1,n
         do iy=1,n
            do ix=1,n
               q(ix,iy,iz) = emo(ix,iy,iz)*(qi(ix,iy,iz)+dt2*psi(ix,iy,iz))  
               qf(ix,iy,iz) = qi(ix,iy,iz) + dt6*psi(ix,iy,iz)
            enddo
         enddo   
      enddo   
!$omp end parallel do
...
!$omp parallel do private(ix,iy,iz,tmp) shared(q,emo,psi,qi,epo,qf)
      do iz=1,n
         do iy=1,n
            do ix=1,n
               tmp = epo(ix,iy,iz)*psi(ix,iy,iz)
               q(ix,iy,iz) = emo(ix,iy,iz)**2*(qi(ix,iy,iz)+dt2*tmp)  
               qf(ix,iy,iz) = qf(ix,iy,iz) + dt3*tmp
            enddo
         enddo   
      enddo   
!$omp end parallel do
...
!$omp parallel do private(ix,iy,iz,tmp) shared(q,emo,psi,qi,epo,qf)
      do iz=1,n
         do iy=1,n
            do ix=1,n
               tmp = epo(ix,iy,iz)**2*psi(ix,iy,iz)
               q(ix,iy,iz) = emo(ix,iy,iz)**2*(qi(ix,iy,iz)+dt*tmp)  
               qf(ix,iy,iz) = qf(ix,iy,iz) + dt3*tmp
            enddo
         enddo   
      enddo   
...
!$omp parallel do private(ix,iy,iz,tmp) shared(q,emo,psi,qi,epo,qf)
      do iz=1,n
         do iy=1,n
            do ix=1,n
               tmp = dt6*epo(ix,iy,iz)*psi(ix,iy,iz)
               q(ix,iy,iz) = emo(ix,iy,iz)**2*(qf(ix,iy,iz)+tmp)  
            enddo
         enddo   
      enddo   
!$omp end parallel do
